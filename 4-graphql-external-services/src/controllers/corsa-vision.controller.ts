...
import {GeoPoint} from '../services/geo-code.service';
...
export class CorsaVisionController {
  ...
  async create(@requestBody() corsaVision: CorsaVision): Promise<CorsaVision> {
    if (corsaVision.address) {
      const geoPoints = await this.geoCodeService.geocode(corsaVision.address);

      if (geoPoints.length) {
        corsaVision.coordinates = geoPoints[0];
      }
    }
